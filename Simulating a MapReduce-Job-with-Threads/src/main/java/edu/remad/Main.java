...
public class Main {
	...
	private static final List<Map.Entry<String, Integer>> intermediateResult = Collections
			.synchronizedList(new ArrayList<>());
	private static final List<List<Map.Entry<String, Integer>>> reducersInput = Collections
			.synchronizedList(new ArrayList<>());
	...
	static class Partitioner implements Runnable {
		...
		public void run() {
			...
			List<String> uniqueWords = intermediateResult.stream().map(Map.Entry::getKey).distinct()
					.collect(Collectors.toList());

			for (String word : uniqueWords) {
				List<Map.Entry<String, Integer>> reduceInput = intermediateResult.stream()
						.filter(entry -> entry.getKey().equals(word)).collect(Collectors.toList());

				reducersInput.add(reduceInput);
			}
		}
